...
<head>
  ...
  <script src="<?php gec($GLOBALS["default"]->weburl) ?>riches/kit/push_notification/push.js"></script>
    <script src="<?php gec($GLOBALS["default"]->weburl) ?>riches/kit/push_notification/serviceWorker.min.js"></script>




    <?php gr_core('hf', 'header'); ?>
     <?php     include("fcm_push/pushnoti_token.php");   ?>
        ...
        <div class='window fh'>
            <div class="container-fluid fh">
                <div class="row fh">
                    ...
                    <!--//////////////////////////////////////////////////////////////////-->
                    <div class="<?php gec($GLOBALS["grusrlog"]['panelclass'].' '); ?>nomob panel chatuser" no=0 ldt=0 lstseen=0 deactiv=0 >
                                ...
                                <span class='box'>
                                    ...
                                    <?php if (isset($GLOBALS["roles"]['features'][13])) {
                                        ...
                                        <span class="switchuser">
                                            <span class="usrimg" data-toggle="tooltip" title="<?php gec($GLOBALS["lang"]->send_as_user) ?>"><img class="lazyimg userimagepushnotification" ></span>
                                            <span class="uslist">
                                                <span>
                                                    <i class="gi-search"></i>
                                    ...
                                    <i class='sendbtn notifyMe' ><i></i></i>
                    ...
                    <?php if ($GLOBALS["logged"]) {  
                        ...
                        <div class="col-md-5 col-lg-3 nomob aside rside tabfold">
                            <div class='top'>
                                <span class='left'>
                                    <i class='icon gi-left-open goback d-md-none'></i>
                                    <span class="vwp user_login_inactive" no="<?php gec($GLOBALS["user"]['id']); ?>">
                                        <img class="lazyimg loginuserimage" hello="<?php gec(gr_img('users', $GLOBALS["user"]['id'])); ?>" data-src="<?php gec(gr_img('users', $GLOBALS["user"]['id'])); ?>">
...
$ids = "'".$GLOBALS["user"]['id']."'"; 
$sql = "SELECT * FROM gr_msgs where uid = ".$ids;
...
$ids2 = "'".$GLOBALS["user"]['id']."'"; 
$sql2 = "SELECT token_id FROM gr_users where uid = ".$ids2;
...
var textmessage='';
$('body').on('keyup', '.emojionearea > .emojionearea-editor', function(e) {
  textmessage = jQuery('.box > .emojionearea > .emojionearea-editor').text();
...
<script> 
$(document).on('click', '#logout_update123', function() {
   var chatwith = 'ok';
  // alert(chatwith);
});
</script>
 ...
 $chatwith = $_POST['chatwith'];
    setuserlogout($chatwith);
     ...
     function setuserlogout($chatwith){
          $sql = "Update `gr_users` SET `signin_status_id`='0'  WHERE id = " . $GLOBALS["user"]['id'] . "";
          echo("Error description: " . $conn -> error);die();
...
?>
    ...
    <script> 
//$(document).on('click', '#logout_update', function() {
       $("#logout_update").click(function(){
    var inactive_user = $('.user_login_inactive').attr('no');
  //var inactive_user = 'okok22';
  alert(inactive_user);
 var okok = 'helloo';
    $.ajax({
            url: 'chat.php',
            method: 'post',
            data:  { user_login_inactives: inactive_user,okok:okok},
...
// Check connection
    if ($conn->connect_error) {
       ...
       $idssss = "'" . $_POST['user_login_inactives'] . "'";
       if(isset($_POST['user_login_inactives'])){
        $sql = "Update `gr_users` SET `signin_status_id`='0'  WHERE id = " . $idssss . "";
       if ($conn->query($sql) === TRUE) {
